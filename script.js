let data = [
  ...
  },
  {
    ...
    "price": 168,
    "description": "Satisfaction Guaranteed. Return or exchange any order within 30 days.Designed and sold by Hafeez Center in the United States. Satisfaction Guaranteed. Return",
    "category": "jewelery",
  ...
  },
  {
    ...
    "price": 109,
    "description": "Easy upgrade for faster boot up, shutdown, application load and response (As compared to 5400 RPM SATA 2.5” hard drive; Based on published specifications and",
    "category": "electronics",
...
function showdata(value, container) {
  ...
  card.querySelector('.btn').addEventListener('click', () => {
    cartItems.push({ title: value.title, price: value.price });
...
function updateCartSummary() {
  ...
  cartSummary.innerHTML = `
    ...
    <ul>
      ${cartItems.map(item => `<li>${item.title} - $${item.price}</li>`).join('')}
    </ul>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  data.forEach(item => showdata(item, container));
});
